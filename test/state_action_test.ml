...
let tests =
  ...
  >::: [
         ...
         ( "bin test5" >:: fun _ ->
           assert_equal 1
             (Cartpole_state_action.value_to_bin (-3.59) (-4.) 4. 20) );
         ( "bin test6" >:: fun _ ->
         ...
         ( "bin to value test" >:: fun _ ->
           let value =
             Pendulum_state_action.bin_to_value 19
               { low = 0.0; high = 1.0; num_bins = 10 }
           in
           assert_bool "value is close to 1.9"
             (abs_float (1.9 -. value) < 0.0001) );
         ( "pendulum convert" >:: fun _ ->
           test_func 1
             (Pendulum_state_action.convert_state_to_bin [ -1.; -1.; -7.19 ]) );
         ( "pendulum convert1" >:: fun _ ->
           let v =
             Pendulum_state_action.convert_state_to_bin
               [ 1.; -1.25208246222e-008; 0.314987382668 ]
           in
           assert_bool "test" (Int.compare 8000 v = 1) );
         ( "pendulum convert2" >:: fun _ ->
           let v =
             Pendulum_state_action.convert_state_to_bin
               [ 0.999993687422; -0.00355318389298; 0.152762888634 ]
           in
           assert_equal 7790 v );
         ( "pendulum convert3" >:: fun _ ->
           let v =
             Pendulum_state_action.convert_state_to_bin
               [ 0.999896233083; -0.01440566092; -0.0835999333127 ]
           in
           print_int v;
           assert_equal 7789 v );
       ]
